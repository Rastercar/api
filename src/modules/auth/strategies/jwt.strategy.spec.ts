...
describe('JWT Strategy', () => {
  const authTokenServiceMock = { getUserFromDecodedTokenOrFail: jest.fn() }
  ...
  it('Throws a UnauthorizedException when authTokenService fails', async () => {
    authTokenServiceMock.getUserFromDecodedTokenOrFail.mockRejectedValueOnce(new UnauthorizedException())
